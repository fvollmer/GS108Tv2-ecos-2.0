...
#define RAMFS_MAGIC_NUMBER		0x8ba60e35
#define RAMFS_FLASH_ADDR    0xb8800000

struct ramfs_runtime_struct
{
	ramfs_block *block_free_list; // = NULL
...
__externC ramfs_runtime_struct __ramfs_runtime_data;
...
static void block_init(void)
{
    ...
    if( !__ramfs_runtime_data.initialized )
    {
        for( i = 0; i < CYGNUM_FS_RAM_BLOCKS_ARRAY_SIZE; i++ )
        {
            ...
            *(ramfs_block **)b = __ramfs_runtime_data.block_free_list;
           __ramfs_runtime_data.block_free_list = b;
        }
        __ramfs_runtime_data.initialized = true;

			    // recalculate the crc
			    // calculate the crc on the runtime data block
			    //__ramfs_runtime_data.ulgCRC = calc_runtime_crc();
    }
}
...
static ramfs_block *block_alloc(void)
{
    ...
    //b = block_free_list;
    b = __ramfs_runtime_data.block_free_list;

    // and advance list
    if( b != NULL )
        __ramfs_runtime_data.block_free_list = *(ramfs_block **)b;
...
static int ramfs_mount    ( cyg_fstab_entry *fste, cyg_mtab_entry *mte )
{
    ...
    int err;
#ifdef CYGPKG_FS_RAM_BLOCKS_ARRAY
    if (__ramfs_runtime_data.ulgMagicNumber != RAMFS_MAGIC_NUMBER ) {
        __ramfs_runtime_data.initialized = false; /* re-init */
        __ramfs_runtime_data.block_free_list = NULL;
#endif
...
#ifdef CYGPKG_FS_RAM_BLOCKS_ARRAY    
    __ramfs_runtime_data.ulgMagicNumber = RAMFS_MAGIC_NUMBER;
    __ramfs_runtime_data.root_node = root;
    
    } else {
        mte->root = (cyg_dir)(__ramfs_runtime_data.root_node);
        __ramfs_runtime_data.initialized = true;
    }
#endif    
    return ENOERR;
