...
// Contributors: 
// Date:         
// Purpose:      BCM953000 platform IO support
// Description: 
...
// Secondary PCI/PCIE port (optional)
#define HAL_PCI_SECONDARY_PORT_BUS_START    17
#define HAL_PCI_ALLOC_SECONDARY_MEMORY      0x40000000
#define HAL_PCI_ALLOC_SECONDARY_IO          0x00000000

// This is where the PCI spaces are mapped in the CPU's address space.
#define HAL_PCI_PHYSICAL_MEMORY_BASE        0xA0000000
#define HAL_PCI_PHYSICAL_IO_BASE            0xA0000000

// Max number of PCI buses supported
#define CYG_PCI_MAX_BUS         ((HAL_PCI_SECONDARY_PORT_BUS_START - 1) * 2 + 1)

// Limit it for performance (but actually only bus 0 has more than 2 devices)
#define CYG_PCI_MAX_DEV                     16
...
#define HAL_PCI_TRANSLATE_INTERRUPT( __bus, __devfn, __vec, __valid) \
    ...
    if (0 != __req) {                                                         \
        if (__bus == 0) {                                                     \
           ...
           __vec = __bus >= HAL_PCI_SECONDARY_PORT_BUS_START?                 \
                        CYGNUM_HAL_INTERRUPT_PCIE_PORT1 :                     \
                        CYGNUM_HAL_INTERRUPT_PCIE_PORT0;                      \
        }                                                                     \
        __valid = true;                                                       \
    } else {                                                                  \
