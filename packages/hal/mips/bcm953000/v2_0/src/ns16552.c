...
#define UART_DBG_MEMORY_DUMP_ADDR   (0) /* disabled if 0 */   
#if (UART_DBG_MEMORY_DUMP_ADDR != 0)
static char *ptrace = (char *)UART_DBG_MEMORY_DUMP_ADDR;
#endif /* UART_DBG_MEMORY_DUMP_ADDR */

/* Interrupt mode */
#define UART_INTERRUPT_MODE
#define UART_INTERRUPT_RX_BUFFER_LEN    (512)

#define NO_NETWORK_IO
...
#ifdef FEAT_3COM_RANGER48_CLI
static bool timeout = 0;
static int time_value = 0xffffffff;
void timeout_interval_set_3com(int t)
{
    time_value = t;
}

void timeout_flag_set_3com(bool flag)
{
    timeout = flag;
}

void timeout_flag_get_3com(bool *flag)
{
    *flag = timeout;
}
#endif /* FEAT_3COM_RANGER48_CLI */
...
{
        bool tmp;
        tmp = cyg_semaphore_timed_wait(&chan->sem_received, cyg_current_time()+time_value);
        if (!tmp) {
            timeout_flag_set_3com(true);
        }
}        
#else /* FEAT_3COM_RANGER48_CLI */
...
{
    if (num_tty >= sizeof(ns_ser_channels) / sizeof(channel_data_t)) {
        return;
    }
