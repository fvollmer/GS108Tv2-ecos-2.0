 ...
 *
 * $Id: hndmips.c,v 1.3 2009/02/04 10:45:18 cchao Exp $
 */
...
#include <cyg/hal/mips33_core.h>
#include <cyg/hal/mips74k_core.h>
#include <cyg/hal/hndcpu.h>
	...
	if ((regs = si_setcore(sih, MIPS74K_CORE_ID, 0)) != NULL) {
		if (!oldirq)
			AND_REG(osh, &((mips74kregs_t *)regs)->intmask[0], ~(1 << flag));
                if ((irq == 0) || (irq == 2))
                        OR_REG(osh, &((mips74kregs_t *)regs)->intmask[irq], 1 << flag);
                else {
