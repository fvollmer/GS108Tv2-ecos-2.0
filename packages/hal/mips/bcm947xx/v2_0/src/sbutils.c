...
#define SBCC_GPIO_EN_ADDR  \
        ((uint *)OSL_UNCACHED(SB_ENUM_BASE + OFFSETOF(chipcregs_t, gpioouten)))
#define SBCC_GPIO_CTRL_ADDR  \
    ((uint *)OSL_UNCACHED(SB_ENUM_BASE + OFFSETOF(chipcregs_t, gpiocontrol)))
...
bad:
	SB_ERROR(("sb_doattach: error happened\n"));	
...
sb_gpiocontrol(void *sbh, uint32 mask, uint32 val)
{
    ...
    if ((SB_INFO(sbh))->gpioid == SB_CC) {
		...
		W_REG(SBCC_GPIO_CTRL_ADDR, (R_REG(SBCC_GPIO_CTRL_ADDR) & ~mask) | val);
		return R_REG(SBCC_GPIO_CTRL_ADDR);
...
sb_gpioouten(void *sbh, uint32 mask, uint32 val)
{
    ...
    if ((SB_INFO(sbh))->gpioid == SB_CC) {
		...
		W_REG(SBCC_GPIO_EN_ADDR, (R_REG(SBCC_GPIO_EN_ADDR) & ~mask) | val);
		return R_REG(SBCC_GPIO_EN_ADDR);
...
sb_EBbus_enable(void *sbh, int data_type)
{
	...
	switch (si->gpioid) {
	case SB_CC:
		regoff = OFFSETOF(chipcregs_t, pcmcia_config);
		break;
	}

        if (data_type == DATA_BIT_16){
            sb_corereg(sbh, si->gpioidx, regoff, 0xffffffff, 0x11);
        } else if (data_type == DATA_BIT_8) {
            sb_corereg(sbh, si->gpioidx, regoff, 0xffffffff, 0x1);
        }
        regoff = OFFSETOF(chipcregs_t, pcmcia_memwait);        
