...
/* debug/trace */
#define	DMA_ERROR(args)    diag_printf args
	...
	while ((p = dma_getnextrxp(di, FALSE))) {
		...
		if (skiplen > 0) {
			...
			continue;
                ...
                HAL_DCACHE_INVALIDATE(PKTDATA(di->drv, p), di->rxbufsize);
...
#if 1
		if (di->rxp[rxout] != NULL)
			PKTFREE(di->drv, di->rxp[rxout], 0);
#endif
		di->rxp[rxout] = p;
