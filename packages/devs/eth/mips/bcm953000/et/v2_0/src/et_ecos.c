...
#endif
#include CYGBLD_DEVS_ET_DEVICE_H
...
static et_info_t et0_priv_data;
ETH_DRV_SC(et_sc0,
...
NETDEVTAB_ENTRY(et_netdev0, 
                ...
                et_drv_init, 
                &et_sc0);
#endif /* CYGPKG_DEVS_ETH_ET0 */

#ifdef CYGPKG_DEVS_ETH_ET1
static et_info_t et1_priv_data;
ETH_DRV_SC(et_sc1,
           &et1_priv_data,      /* Driver specific data */
           ...
           et_poll,
           et_int_vector);
NETDEVTAB_ENTRY(et_netdev1, 
                "dev_et1", 
                et_drv_init, 
                &et_sc1);
#endif /* CYGPKG_DEVS_ETH_ET1 */

static et_info_t* et_info_array[] = {
#ifdef CYGPKG_DEVS_ETH_ET0
    &et0_priv_data,
#endif /* CYGPKG_DEVS_ETH_ET0 */
#ifdef CYGPKG_DEVS_ETH_ET1
...
/* static functions */
static cyg_uint32 et_isr(cyg_vector_t vector, cyg_addrword_t data);
...
et_drv_init(struct cyg_netdevtab_entry * ndp)
{
    ...
    if (initialized == 0) {
        ...
        for (device_index = 0; 
             device_index < sizeof(et_info_array)/sizeof(et_info_t *);
             device_index++) {
            
            et_info_t *et_info = et_info_array[device_index];
            memset(et_info, '\0', sizeof(et_info_t));
            ...
            if (cyg_pci_find_matching (&et_find_match, NULL, &devid)) {
                ...
                if (cyg_pci_configure_device(et_info->pdev)) {
                    ...
                    osh = osl_attach(dev_info, PCI_BUS, FALSE);
                    if (osh == NULL) {
                        return 0;
                    }
                    ...
                    et_info->active = 0;
                    dprintf("et_init: failed to initialize device %d\n",
                             device_index);
                }
            } else {
                et_info->active = 0;
                diag_printf("et_init: failed to find device %d\n", device_index);
            }
        } /* for */

        /* Meaning the system initialization/discovery phase is done */
        initialized = 1;    
    ...
    /* If not initialized (device not attached), quit */
    if (et->etc == NULL) {
        return 0;
    }

    /* netdev table backtrace */
    et->ndp = ndp;
    ...
    (sc->funs->eth_drv->init)(sc, (unsigned char *)&et->etc->cur_etheraddr);
...
et_isr(cyg_vector_t vector, cyg_addrword_t data)
{
    ...
    events = (*et->etc->chops->getintrevents)(et->etc->ch, true);
...
et_dsr(cyg_vector_t vector, cyg_ucount32 count, cyg_addrword_t data)
{
    ...
    dprintf("et_dsr: vector=%u count=%u data=%p\n", vector, count, (void *)data);
...
err:
    bcm_ether_ntoa((const struct ether_addr *)((struct ether_header*)sg->data)->ether_shost, eabuf);
    if (flags & RXF_NO) {
            ...
            unsigned long key)
{
    ...
    pkt = PKTGET (0, total_len, 0, MEMORY_DDRRAM);
...
et_recv( struct eth_drv_sc *sc, struct eth_drv_sg *sg_list, int sg_len )
{
    ...
    for ( last_sg = &sg_list[sg_len]; sg_list < last_sg; sg_list++ ) {
        ...
        if ( 0 == to_p ) {
            PKTFREE(0, et->rxpkt,0, MEMORY_DDRRAM);
