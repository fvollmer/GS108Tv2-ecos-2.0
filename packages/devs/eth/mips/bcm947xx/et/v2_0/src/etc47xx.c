...
chipattach(etc_info_t *etc, void *dev, void *regsva)
{
	...
	/* get sb handle */
	if ((ch->sbh = sb_attach(etc->deviceid, ch->dev, regsva, PCI_BUS, NULL, &ch->vars, &ch->vars_size)) == NULL) {
	...
	if (var == NULL) {
	    /*
	     * Let's give it a dummy default value to prevent init failure
	     * because it can never be brought up if init failed.
	     *
	     * In this case, factory defaults will (should) overwrite it later.
	     */
	    var = "00-10-18-58-36-00";
	}
	...
	if ((etc->chip == 0x4307) && (etc->chiprev == 2)) {
		sbpciregs_t *pciregs;
		if ((pciregs = (sbpciregs_t *)sb_setcore(ch->sbh, SB_PCI, 0)) != NULL) {
			...
			regs = (bcmenetregs_t *)sb_setcore(ch->sbh, SB_ENET, 0);
			ASSERT(sb_coreid(ch->sbh) == SB_ENET);
		}
	}
#endif
	...
	if ((boardflags & BFL_ENETSPI) || (boardtype == BCM94710AP_BOARD)
		...
		if (!(ch->robo = robo_attach(ch->sbh, (1 << 2), (1 << 3), (1 << 4), (1 << 5)))) {
...
#ifdef  AFTER_VLAN_IMPL  /* enable this part of code after vlan impl */
