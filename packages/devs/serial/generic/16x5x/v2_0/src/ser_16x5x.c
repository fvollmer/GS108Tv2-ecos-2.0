...
#if 0 /* ALOK */
cyg_thread thread_uart[1];     /* space for two thread objects */

char stack_uart[1][4096];        /* space for two 4K stacks */

/* now the handles for the threads */
cyg_handle_t uarthandle;

/* and now variables for the procedure which is the thread */
cyg_thread_entry_t uart_entry;
#endif

#define CYGDBG_IO_INIT 1  /* ALOK */
...
pc_serial_init(struct cyg_devtab_entry *tab)
{
...
#endif
	HAL_INTERRUPT_MASK(ser_chan->int_num);
    HAL_WRITE_UINT8(ser_chan->base+REG_ier, 0);
    HAL_WRITE_UINT8(ser_chan->base+REG_mcr, 0);
    ...
    if (chan->out_cbuf.len != 0) {
        ...
        cyg_drv_interrupt_configure(ser_chan->serial_interrupt_handle, 0,0);
        
        cyg_drv_interrupt_unmask(ser_chan->int_num);
/* Enable the below code to create polled based fully functional serial driver */
        cyg_thread_create(10, uart_entry, (cyg_addrword_t)chan ,
	              "UART_THREAD", (void *) stack_uart[0], 4096,
				   &uarthandle, &thread_uart[0]);
#endif
    }
#if 0 /* alok */
    serial_config_port(chan, &chan->config, true);
#endif
	HAL_INTERRUPT_UNMASK(ser_chan->int_num);
    return true;
}
...
pc_serial_putc(serial_channel *chan, unsigned char c)
{
    ...
    do {
       ...
       } while ((_lsr & LSR_THE) == 0);

        HAL_WRITE_UINT8(base+REG_thr, c);
        return true;
}

int pc_test_out(unsigned char c)
{
    cyg_uint8 _lsr;
    cyg_addrword_t base = (cyg_addrword_t)0xb8000300;
...
void uart_entry(cyg_addrword_t data)
{
  cyg_uint8 _lsr;
  unsigned char c;
  serial_channel *chan = (serial_channel *)data;
