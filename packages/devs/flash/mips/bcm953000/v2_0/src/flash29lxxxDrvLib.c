...
flashAutoSelect(FLASH_TYPES *dev, FLASH_VENDORS *vendor)
{
    ...
    *dev = FLASH_READ(FLASH_BASE_ADDRESS, 2);
    ...
    if (*dev == FLASH_2L640) {
        /*
         * Special case for 29VL640 series:
         * It shares the same device ID as 29GL series,
         * so we do one step further to make sure which one we're using.
         */
        FLASH_WRITE(FLASH_BASE_ADDRESS, 0xaa, 0x98); /* CFI query */
        if (FLASH_READ(FLASH_BASE_ADDRESS, 0x4E) != 0x17) {
            /* 29LV640 series have a device size == 0x17 */
            *vendor = *dev = 0xFF;
        }
        flashReadReset();   
    }
...
flashEraseSector(int sectorNum)
{
    ...
    if (sectorNum == 0) {
        ...
        /* The sub sector is 8K bytes */
        for(i=1; i<FLASH_SECTOR_SIZE/0x2000; i++) {
            if (flashEraseDevices(sectorBasePtr + i * 0x2000) == ERROR) {
