...
/*
 * DHCP timeout: 4, 8, 16, 32, 64, 128, ....
 */
#define DHCP_PROCESS_TIMEOUT    (64)

/*
 * Variables for ability to cancel DHCP process
 *
 * XXX: This code is built on top of the current limitation that
 *      DHCP works with only one interface up and running.
 */
static int socket_for_dhcp = -1; /* Not in progress or cancelled */
...
{
    /*
     * If the process is cancelled (the socket is closed),
     * return false immediately.
     */
    if (socket_for_dhcp == -1) {
        return false;
    }
    ...
    return pstate->secs < (DHCP_PROCESS_TIMEOUT - DHCP_PROCESS_TIMEOUT / 4);
}
...
#define NEW_XID(_xid) CYG_MACRO_START (_xid)+= 0x10000; CYG_MACRO_END
    ...
    /*
     * Save the socket in case we need to cancel the process
     */
    socket_for_dhcp = s;
...
// ------------------------------------------------------------------------
// Cancel DHCP process if it's not yet bound
void
cancel_dhcp_process(void)
{
    if (socket_for_dhcp >= 0) {
        int s;
        
        s = socket_for_dhcp;
        socket_for_dhcp = -1;
        
        /* To force any operation on this socket return immediately */
