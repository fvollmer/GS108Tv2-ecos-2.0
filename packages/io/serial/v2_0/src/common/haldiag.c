...
haldiag_getc(serial_channel *chan)
{
	...
	hal_virtual_comm_table_t* mychan = CYGACC_CALL_IF_DEBUG_PROCS();

	if (mychan)
	{
		CYGACC_COMM_IF_CONTROL(*mychan, __COMMCTL_SET_TIMEOUT, 0);
		
		while(!ret) {
			ret = CYGACC_COMM_IF_GETC_TIMEOUT(*mychan, &c);
		}
	}
	else {
