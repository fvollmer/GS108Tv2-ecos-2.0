...
static struct flashiodev_priv_t flashiodev_priv[4];
...
flashiodev_init( struct cyg_devtab_entry *tab )
{
    ...
    if ( stat == 0 ) {
        ...
        else if(!strcmp(tab->name,"/dev/flash2")) 
        {
         dev->start = (char *)flash_info.start + CYGNUM_IO_FLASH_BLOCK_OFFSET_2;
         dev->end = (char *)flash_info.start + CYGNUM_IO_FLASH_BLOCK_OFFSET_2 +
            CYGNUM_IO_FLASH_BLOCK_LENGTH_2;
        }
        else if(!strcmp(tab->name,"/dev/cfe")) 
        {
         dev->start = (char *)flash_info.start + CYGNUM_IO_FLASH_BLOCK_OFFSET_4;
         dev->end = (char *)flash_info.start + CYGNUM_IO_FLASH_BLOCK_OFFSET_4 +
            CYGNUM_IO_FLASH_BLOCK_LENGTH_4;
        }
        else
        {
         dev->start = (char *)flash_info.start + CYGNUM_IO_FLASH_BLOCK_OFFSET_3;
...
BLOCK_DEVTAB_ENTRY( cyg_io_flashdev2,
                    ...
                    0, // No lookup required
                    &flashiodev_priv[1] );
BLOCK_DEVTAB_ENTRY( cyg_io_flashdev3,
                    CYGDAT_IO_FLASH_BLOCK_DEVICE_NAME_3,
                    0,
                    &cyg_io_flashdev1_ops,
                    &flashiodev_init,
                    0, // No lookup required
                    &flashiodev_priv[2] );
BLOCK_DEVTAB_ENTRY( cyg_io_flashdev4,
                    CYGDAT_IO_FLASH_BLOCK_DEVICE_NAME_4,
                    0,
                    &cyg_io_flashdev1_ops,
                    &flashiodev_init,
                    0, // No lookup required
